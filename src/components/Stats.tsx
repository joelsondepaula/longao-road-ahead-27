      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
          {stats.map((stat, index) => (
            <div 
              key={stat.label}
              className="text-center p-6 rounded-2xl bg-card border border-border hover:shadow-lg transition-all duration-300 group"
              style={{ 
        ...
        <div className="text-center mt-12">
          <div className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-premium-gold/20 to-road-yellow/20 rounded-full border border-premium-gold/30">
            <div className="w-3 h-3 bg-success-green rounded-full animate-pulse mr-3"></div>
            <span className="text-sm font-medium text-foreground">
              Jornada em andamento • Atualizações em tempo real
            </span>
          </div>
        </div>
      </div>
